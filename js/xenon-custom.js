...
function date(format, timestamp) {
	...
	return this.date(format, timestamp);
		...
		function activateSliders(){
			...
			$("body .slider").each(function(i, el)
			{
				var $this = $(el),
					...
					$this.slider({
						...
						change: function(ev, ui)
						{
							if(reps == 1)
							{
								...
								$label_1.html( val );
								var setName = $(this).attr("data-id");	
								var index = $(this).closest("div.panel-collapse").index(".panel-collapse");
								$("div#wrapper").bannerBuilder("set"+setName,index,val);
								if(fill)
									$fill.val(val);
							}
